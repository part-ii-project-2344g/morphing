...
def write_file(file, measurements):
    for i in range(len(measurements)):
        for j in range(i+1, len(measurements)):
            ...
            and not measurements[i][IND_IS_R_Z]) and measurements[j][IND_IS_R_Z]) \
            or ((not measurements[j][IND_IS_R_X] and not measurements[j][IND_IS_R_Y] \
            and not measurements[j][IND_IS_R_Z]) and measurements[i][IND_IS_R_Z])) \
            and True:
                rot_val   = measurements[i][IND_VALUE] if measurements[i][IND_IS_R_Z] else measurements[j][IND_VALUE]
                norot_val = measurements[j][IND_VALUE] if measurements[i][IND_IS_R_Z] else measurements[i][IND_VALUE]
