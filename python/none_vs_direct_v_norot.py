...
def write_file(file, measurements):
    for i in range(len(measurements)):
        for j in range(i+1, len(measurements)):
            ...
            and measurements[i][IND_METHOD]+"|"+measurements[j][IND_METHOD] in ["none|direct", "direct|none"] \
            and measurements[i][IND_IS_VOL] \
            and measurements[j][IND_IS_VOL] \
            and not measurements[i][IND_IS_R_X] \
            and not measurements[j][IND_IS_R_X] \
            and not measurements[i][IND_IS_R_Y] \
            and not measurements[j][IND_IS_R_Y] \
            and not measurements[i][IND_IS_R_Z] \
            and not measurements[j][IND_IS_R_Z] \
            and True:
                none_val   = measurements[i][IND_VALUE] if measurements[i][IND_METHOD] == "none" else measurements[j][IND_VALUE]
                direct_val = measurements[j][IND_VALUE] if measurements[i][IND_METHOD] == "none" else measurements[i][IND_VALUE]
                file.write(str(none_val)+","+str(direct_val)+"\n")
