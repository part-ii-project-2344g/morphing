...
axis1 = "M - original"
axis2 = "M - one mesh rotated 80 deg. around x-axis"
HEADER = axis1+","+axis2
...
def write_file(file, measurements):
    for i in range(len(measurements)):
        for j in range(i+1, len(measurements)):
            ...
            and not measurements[i][IND_IS_R_Z]) and measurements[j][IND_IS_R_X]) \
            or ((not measurements[j][IND_IS_R_X] and not measurements[j][IND_IS_R_Y] \
            and not measurements[j][IND_IS_R_Z]) and measurements[i][IND_IS_R_X])) \
            and True:
                rot_val   = measurements[i][IND_VALUE] if measurements[i][IND_IS_R_X] else measurements[j][IND_VALUE]
                norot_val = measurements[j][IND_VALUE] if measurements[i][IND_IS_R_X] else measurements[i][IND_VALUE]
                file.write(str(norot_val)+","+str(rot_val)+"\n")
